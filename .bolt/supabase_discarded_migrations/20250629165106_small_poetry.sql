/*
  ...
  1. Changes
    - Properly drops all functions before recreating them
    - Fixes return type issues with get_user_notifications
    - Ensures all function signatures are updated correctly
    - Adds missing conversation and messaging functions
*/
...
-- Create trigger for update_conversation_last_message if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create trigger for mark_message_edited if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
...
-- Create trigger for booking_approval_notification_trigger if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
