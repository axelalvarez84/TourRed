...
async function redirectForUser(user: any, navigate: (path: string, opts?: any) => void) {
  ...
  if (isFacebookProvider) {
    ...
    if (!onboardingCompleted) {
      ...
      if (existingProfile) {
        // User linked Facebook to an existing account — save avatar if not set yet
        ...
        navigate('/auth/facebook-onboarding', { replace: true });
      }
      return;
    }
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500">Completando inicio de sesión con Facebook...</p>
      </div>
    </div>
  );
};
