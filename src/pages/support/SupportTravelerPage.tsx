...
import { User, CheckCircle, ArrowLeft, Loader, TicketCheck } from 'lucide-react';
...
const SupportTravelerPage: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      const [catsRes, subsRes, profileRes] = await Promise.all([
        ...
        user ? supabase.from('users').select('first_name, last_name, email').eq('id', user.id).maybeSingle() : null,
      ]);
      setCategories(catsRes.data ?? []);
      const travelerSubs = (subsRes.data ?? []).filter(s => s.aplica_a?.includes('traveler'));
      setSubcategories(travelerSubs);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasErrors) { setError('Hay archivos con errores.'); return; }
    ...
    try {
      ...
      formData.append('data', JSON.stringify({
        ...
        solicitante_nombre: userProfile ? `${userProfile.first_name} ${userProfile.last_name}` : user?.email,
        solicitante_email: userProfile?.email ?? user?.email,
        descripcion,
        user_id: user?.id,
      }));
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-4">Tu solicitud de soporte fue registrada exitosamente.</p>
          ...
          <p className="text-sm text-gray-500 mb-6">
            Puedes dar seguimiento a tus tickets desde tu panel de viajero.
          </p>
          <div className="flex flex-col gap-3">
            <button onClick={() => navigate('/traveler/soporte')} className="btn btn-primary flex items-center justify-center gap-2">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="container-custom py-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Soporte para Viajeros</h1>
              <p className="text-gray-500 text-sm">
                {userProfile ? `${userProfile.first_name} ${userProfile.last_name} — ${userProfile.email}` : user?.email}
              </p>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {categoryId && (
              <div>
                ...
                {filteredSubs.length === 0 && (
                  <p className="text-xs text-gray-400 mt-1">No hay opciones disponibles para esta categoria.</p>
                )}
              </div>
            )}
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe detalladamente tu problema o duda..."
                required
              />
...
export default SupportTravelerPage;
