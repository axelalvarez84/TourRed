...
const InsuranceCommissionModal: React.FC<Props> = ({ onClose, onSaved }) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.period_start || !form.period_end) {
      setError('Debes especificar el periodo de la comisión.');
      return;
    }
    ...
    setSaving(true);
    const { error: dbErr } = await supabase.from('insurance_commission_receipts').insert({
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Registrar Comisión</h2>
              <p className="text-sm text-gray-500">Comisión recibida de aseguradora (Flujo C)</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Monto comisión (MXN)</label>
              <input
                type="number"
                step="0.01"
                min="0.01"
...
export default InsuranceCommissionModal;
