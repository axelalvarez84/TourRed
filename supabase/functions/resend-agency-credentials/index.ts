...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!agencyId) {
      return new Response(
        JSON.stringify({ error: "Falta el campo agencyId" }),
    ...
    if (agency.onboarding_status === "active") {
      return new Response(
        JSON.stringify({ error: "Esta agencia ya firmó su contrato y no se pueden reenviar credenciales" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    if (authUpdateError) {
      return new Response(
        JSON.stringify({ error: "Error al actualizar la contraseña: " + authUpdateError.message }),
    ...
    console.error("Error in resend-agency-credentials:", error);
    return new Response(
