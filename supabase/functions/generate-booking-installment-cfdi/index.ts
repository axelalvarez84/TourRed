  ...
  try {
    ...
    try {
      ...
      if (stampErrStr.includes("CFDI40192") || stampErrStr.includes("DomicilioFiscalACuentaTerceros") || stampErrStr.includes("40192")) {
        ...
        try {
          cfdiResult = await stampCfdi(settings.pac_provider, settings.pac_api_key_encrypted!, settings.pac_organization_id || "", retryRequest, settings.pac_sandbox_mode, supabase);
        } catch (retryError) {
          ...
          return new Response(JSON.stringify({ error: "Error al timbrar con PAC (retry sin tercero tambien fallo)", detail: String(retryError) }), {
            status: 502,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
