/*
  # Notificaciones al ejecutivo por eventos clave

  ## Descripción
  Amplía los triggers existentes (handle_agency_approved, handle_tour_published,
  handle_booking_paid) para que, además de generar comisiones, envíen:
    1. Un email al ejecutivo via pg_net → Edge Function send-executive-notification
    2. Una notificación en la app (tabla notifications)

  También agrega el valor 'commission_earned' al enum notification_type.

  ## Eventos cubiertos
  - Agencia aprobada → comisión de aprobación + email + notificación en app
  - Primer tour publicado → email + notificación en app
  - Primera reserva pagada → comisión first_tour_and_booking + email + notificación en app

  ## Notas
  - pg_net es asíncrono: si el email falla no revienta la transacción principal
  - La URL de la Edge Function se lee desde email_settings.supabase_project_url o
    se construye con SUPABASE_URL (disponible en funciones SECURITY DEFINER via GUC)
*/
