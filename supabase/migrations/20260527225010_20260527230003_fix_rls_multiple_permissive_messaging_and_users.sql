/*
  # Fix Multiple Permissive Policies — Mensajes, Usuarios y Referidos

  ## Problema
  Múltiples políticas permisivas para el mismo rol y acción se evalúan todas
  y se combinan con OR, generando overhead innecesario.

  ## Solución
  Consolidar N políticas del mismo rol/acción en 1 sola política con OR.
  La lógica de acceso no cambia.

  ## Tablas modificadas
  1.  messages — INSERT (2→1), SELECT (2→1)
  2.  message_participants — SELECT (2→1)
  3.  notifications — SELECT (2→1)
  4.  payment_transactions — SELECT (2→1)
  5.  referral_bonuses — SELECT (2→1)
  6.  referral_codes — SELECT (2→1), UPDATE (2→1)
  7.  referral_relationships — SELECT (2→1)
  8.  terms_acceptances — SELECT (2→1)
  9.  toursred_cash_transactions — SELECT (2→1)
  10. toursred_cash_wallets — SELECT (2→1)
  11. toursred_points_transactions — SELECT (2→1)
  12. toursred_points_wallets — SELECT (2→1)
  13. users — SELECT (5→1), UPDATE (2→1)
*/
