/*
  # Create Booking Cancellations System
  ...
  1. New Tables
    - `booking_cancellations`
      ...
      - `days_before_tour` (integer) - Days remaining before tour at cancellation time
      - `cancellation_policy_type` (text) - Type: '100_percent', '50_percent', 'no_refund', 'no_show', 'pending_approval'
      - `original_deposit_amount` (numeric) - Original deposit paid
      - `original_service_charge` (numeric) - Original service charge paid
      - `refund_amount_to_traveler` (numeric) - Amount refunded to traveler in ToursRed Cash
      - `amount_to_agency` (numeric) - Amount agency will receive
      - `amount_to_platform` (numeric) - Platform commission retained
  ...
  2. New Columns in `tours`
    - `cancellation_not_allowed` (boolean, default false) - Tour doesn't allow cancellations with refund

  3. New Columns in `bookings`
    - `cancelled_at` (timestamptz, nullable)
    - `cancellation_type` (text, nullable)
    - `cancellation_refund_amount` (numeric, nullable)

  4. Security
    - Enable RLS on `booking_cancellations` table
    - Add policies for authenticated users to view their own cancellations
    - Add policies for agencies to view their tour cancellations
