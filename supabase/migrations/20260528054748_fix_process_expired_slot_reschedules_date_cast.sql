...
BEGIN
  -- -----------------------------------------------------------------------
  -- Paso 1: Procesar solicitudes con deadline expirado
  -- -----------------------------------------------------------------------
  FOR v_request IN
    SELECT srr.*
  ...
  LOOP
    ...
    LOOP
      ...
      -- Cancelar la reserva (dispara trigger de comisiones automaticamente)
      UPDATE bookings
      SET
        status                       = 'cancelled',
        cancelled_at                 = now(),
    ...
    -- Actualizar contadores de la solicitud
    ...
    -- Mover al slot destino las reservas que SI aceptaron manualmente antes del deadline
  ...
  -- -----------------------------------------------------------------------
  -- Paso 2: Completar solicitudes donde todos ya respondieron voluntariamente
  -- -----------------------------------------------------------------------
  FOR v_request IN
    SELECT srr.*
    FROM slot_reschedule_requests srr
