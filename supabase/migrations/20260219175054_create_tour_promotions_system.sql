/*
  # Create Tour Promotions System

  ## Summary
  This migration creates a complete group promotions system for tours, including:
  - 2x1 (buy 2, pay 1)
  - 3x2 (buy 3, pay 2)
  - Custom group fixed price

  ## New Tables
  - `tour_promotions`
    - `id` (uuid, primary key)
    - `tour_id` (uuid, FK to tours)
    - `agency_id` (uuid, FK to agencies, denormalized for RLS)
    - `promotion_type` (enum: '2x1', '3x2', 'grupo_precio_fijo')
    - `min_travelers` (int, minimum travelers to trigger promo)
    - `group_size` (int, size of group in the promo - 2 for 2x1, 3 for 3x2)
    - `pay_count` (int, how many pay within group - 1 for 2x1, 2 for 3x2)
    - `fixed_group_price` (numeric, used for grupo_precio_fijo type)
    - `valid_from` (timestamptz, promo start date)
    - `valid_until` (timestamptz, promo end date)
    - `max_uses` (int, optional usage cap)
    - `times_used` (int, counter)
    - `is_active` (boolean)
    - `deactivation_reason` (text, filled by admin when deactivated for policy violations)
    - `created_by` (uuid, FK to users)
    - `created_at` / `updated_at` (timestamps)

  ## Modified Tables
  - `bookings`: adds `promotion_id` and `promo_discount_amount` fields

  ## Security
  - RLS enabled on `tour_promotions`
  - Agencies can CRUD their own tour promotions
  - Admin can view and update (deactivate) any promotion
