/*
  # Funciones de Polizas Contables Automaticas

  ## Descripcion
  Crea las funciones RPC que generan polizas contables a partir de los eventos
  ya existentes en la plataforma (bookings, payouts, cancelaciones).

  El modelo contable correcto para ToursRed:
  - Al recibir el anticipo del viajero: el dinero entra a Bancos y se registra un PASIVO
    en "Anticipos de clientes" (208) — NO es ingreso todavia
  - Al completar el tour: se devengal el ingreso real — la comision propia (401) y el
    cargo de servicio (402) son los unicos ingresos propios. El resto se mueve a CxP Agencias
  - Al pagar a la agencia: baja el pasivo CxP Agencias y sale de Bancos

  ## Funciones Creadas

  - `create_accounting_entry_for_booking(booking_id)` — poliza de ingreso al confirmar pago
  - `create_accounting_entry_for_tour_completion(commission_record_id)` — poliza al completar tour
  - `create_accounting_entry_for_payout(payout_id)` — poliza de egreso al pagar agencia
  - `create_accounting_entry_for_cancellation(booking_id)` — poliza de reverso por cancelacion
  - `generate_accounting_entries_batch(p_from_date, p_to_date)` — procesa eventos pendientes en lote
  - `get_trial_balance(p_year, p_month)` — balanza de comprobacion para el periodo
  - `get_balance_sheet(p_date)` — balance general a una fecha
  - `get_income_statement(p_from, p_to)` — estado de resultados de un periodo
*/
