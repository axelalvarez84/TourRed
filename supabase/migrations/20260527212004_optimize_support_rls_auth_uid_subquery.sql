/*
  # Optimizar RLS del módulo de soporte: auth.uid() → (select auth.uid())

  ## Problema
  auth.uid() es una función volatile que Postgres re-evalúa en cada fila.
  Envolviendo en (select auth.uid()) se convierte en una subquery escalar
  que el planner evalúa UNA SOLA VEZ por query, reduciendo el costo de RLS
  en tablas con muchas filas.

  ## Tablas modificadas
  1. support_categories — 3 políticas
  2. support_subcategories — 3 políticas
  3. support_tickets — 4 políticas
  4. support_ticket_comments — 4 políticas
