/*
  # Crear tabla de categorías de tours

  1. Nueva Tabla
    - `tour_categories`
      - `id` (uuid, primary key)
      - `name` (text) - Nombre visible de la categoría
      - `slug` (text, unique) - Identificador único en formato slug
  ...
  2. Datos Iniciales
    - Se insertan las categorías predeterminadas:
      - Aventura
      - Naturaleza
      - Cultural
      - Playa
  ...
  3. Seguridad
    - Enable RLS en `tour_categories`
    - Policy para que todos puedan leer categorías activas
    - Policy para que solo admins puedan crear/editar/eliminar
*/
