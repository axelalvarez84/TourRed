/*
  # Correcciones de seguridad y performance — módulos nuevos

  ## Resumen
  Esta migración corrige los hallazgos de seguridad y performance detectados en los módulos
  de wallet check-in, CFDI manual, comisiones de ejecutivos y notificaciones.

  ## Correcciones de SEGURIDAD ALTA

  ### 1. Políticas RLS con WITH CHECK (true) en wallet_checkin_otps y wallet_checkin_charges
  Las inserciones y actualizaciones en estas tablas SOLO ocurren desde edge functions que usan
  service_role (el cual bypasea RLS completamente). Las políticas abiertas para el rol
  `authenticated` son innecesarias y permiten que cualquier usuario autenticado inserte/modifique
  OTPs y registros de cargo ajenos.
  - Eliminadas: "System can insert checkin otps", "System can update checkin otps"
  - Eliminada: "System can insert checkin charges"

  ### 2. GRANT EXECUTE a `authenticated` en funciones que no se llaman desde el frontend
  - `create_accounting_entry_for_manual_cfdi`: solo la llama el edge function generate-manual-cfdi
    con service_role → revocado de authenticated
  - `calculate_executive_platform_commissions`: nunca se llama desde React; la llama
    internamente generate_and_notify_platform_commissions (SECURITY DEFINER, corre como postgres)
    → revocado de authenticated

  ### 3. Validación interna de rol en funciones admin llamadas desde React
  - `get_garbage_bookings`: llamada desde AdminBookingsCleanup.tsx → agrega check de rol admin
  - `generate_and_notify_platform_commissions`: llamada desde AdminEjecutivosComisiones.tsx
    → agrega check de rol admin

  ### 4. Política INSERT de notifications abierta para authenticated
  Las notificaciones siempre las crea el sistema (funciones SECURITY DEFINER o edge functions
  con service_role). Se elimina la política INSERT para authenticated.

  ## Correcciones de PERFORMANCE MEDIA

  ### 5. RLS con auth.uid() directo (re-evaluación por fila)
  - booking_cleanup_logs: cambiado a (SELECT auth.uid())
  - notifications: cambiado a (SELECT auth.uid()) en SELECT y UPDATE

  ### 6. Foreign keys sin índice cubiente
  - cfdi_invoices(accounting_account_code) → chart_of_accounts(code)
  - manual_cfdi_recipients(created_by) → auth.users(id)
  - wallet_checkin_charges(otp_id) → wallet_checkin_otps(id)
*/
