/*
  # Add Agency-Specific Discount Codes with Tour Selection

  1. Schema Changes
    - Add `agency_id` column to `discount_codes` table
      - UUID nullable, references agencies(id) ON DELETE CASCADE
      - Allows agencies to create their own discount codes
    
    - Add `tour_id` column to `discount_codes` table
      - UUID nullable, references tours(id) ON DELETE CASCADE
      - Allows agencies to create codes specific to one tour
      - If NULL, code applies to all tours of the agency
    
    - Add new discount types for agencies:
      - `agency_tour_percentage`: Agency-created percentage discount on tours
      - `agency_tour_fixed`: Agency-created fixed amount discount on tours
    
    - Update constraints:
      - When agency_id is present, discount must be agency_tour type
      - When tour_id is present, agency_id must also be present
      - When tour_id is present, tour must belong to the agency
      - Agency codes must only apply to 'tours'
  
  2. Indexes
    - Add index on agency_id for better query performance
    - Add index on tour_id for better query performance
  
  3. RLS Policies for Agencies
    - Agencies can view their own discount codes
    - Agencies can create codes for their tours (all or specific)
    - Agencies can update their own codes
    - Agencies can delete their own codes
    - Users can view active codes when in tour context
    - If code has tour_id, users only see it for that specific tour
  
  4. Validation Functions
    - Function to validate agency discount codes
    - Function to get agency tours for dropdown
    - Function to validate tour belongs to agency
  
  5. Business Rules Enforced
    - Agency codes only apply to tours owned by that agency
    - Tour-specific codes only apply to that exact tour
    - Codes with tour_id cannot be used on other tours
    - Deleting a tour cascades to delete its specific codes
*/
