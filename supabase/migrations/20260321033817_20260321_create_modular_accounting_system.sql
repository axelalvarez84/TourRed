/*
  # Sistema Contable Modular — Capa de Abstraccion

  ## Resumen
  Implementa la infraestructura de base de datos para el sistema de integracion contable modular.
  Sigue el mismo patron adaptador que ya existe para los PAC de CFDI: una interfaz estandarizada,
  un dispatcher que lee la configuracion y enruta, y adaptadores intercambiables.
  Si se desea cambiar de Zoho Books a Odoo o QuickBooks, solo se cambia el proveedor en
  platform_settings y se implementa el nuevo adaptador — sin tocar la logica de negocio.

  ## Nuevas Tablas

  1. `accounting_sync_log`
     - Registro generoso y proveedor-independiente de todos los eventos de sincronizacion
     - `provider` (text) — proveedor que proceso el registro (zoho_books, odoo, etc.)
     - `record_type` — tipo de registro de negocio (contact, booking, payout, commission, journal)
     - `record_id` — UUID del registro en nuestra plataforma
     - `external_entity_type` — como lo llama el sistema contable externo (Contact, Invoice, etc.)
     - `external_entity_id` — ID del registro en el sistema contable externo
     - `status` — pending / synced / error / skipped
     - `error_message` — detalle del error si status = error
     - `synced_at` — cuando se completo la sincronizacion exitosa
     - `retry_count` — numero de reintentos
     - `payload_summary` — resumen del payload enviado (para debugging, sin datos sensibles)

  2. `accounting_account_mapping`
     - Mapeo configurable del Plan de Cuentas entre nuestra plataforma y cada sistema contable externo
     - Permite cambiar de proveedor actualizando solo este mapeo, sin modificar codigo
     - `provider` — proveedor al que aplica este mapeo
     - `account_key` — nombre semantico interno (ej: 'ingresos_tours', 'iva_trasladado')
     - `external_account_id` — ID de la cuenta en el sistema externo
     - `external_account_name` — nombre de la cuenta en el sistema externo (para referencia)
     - `account_type` — tipo de cuenta (income, expense, asset, liability, equity)
     - `description` — descripcion del uso de esta cuenta

  3. `zoho_oauth_tokens`
     - Almacena los tokens OAuth2 de Zoho Books (access token + refresh token)
     - Access token expira cada hora y se renueva automaticamente usando el refresh token
     - El refresh token es permanente (hasta que se revoque manualmente)

  ## Modificaciones a Tablas Existentes

  4. `platform_settings` — Agrega campos para la integracion contable:
     - `accounting_provider` — proveedor contable activo (zoho_books, odoo, quickbooks, none)
     - `accounting_sync_enabled` — toggle global de sincronizacion en tiempo real
     - `zoho_client_id` — Client ID de la app en Zoho Developer Console
     - `zoho_client_secret` — Client Secret de la app en Zoho (tratado como secreto)
     - `zoho_org_id` — Organization ID de la empresa en Zoho Books
     - `zoho_region` — Region del servidor Zoho (com, eu, in, com.au, jp)
     - `zoho_sandbox_mode` — si true, usa la organizacion de pruebas en Zoho
     - `pac_provider` constraint — se actualiza para incluir 'zoho_books'

  ## Seguridad
  - RLS habilitado en todas las tablas nuevas
  - Solo admins pueden leer/escribir el log de sincronizacion
  - Solo service_role puede insertar/actualizar (via edge functions)
  - Los tokens de Zoho solo son accesibles por service_role (nunca expuestos al cliente)
*/
