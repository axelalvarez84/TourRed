/*
  # Sistema Contable Interno — Tablas Base

  ## Descripcion
  Crea las tablas fundamentales del mini ERP contable:
  - `chart_of_accounts`: Catalogo de cuentas contables alineado al SAT Anexo 24 para RESICO (626)
  - `accounting_entries`: Polizas contables (encabezado)
  - `accounting_entry_lines`: Partidas de cada poliza (debitos y creditos)

  ## Tablas Nuevas

  ### chart_of_accounts
  Catalogo de cuentas con estructura jerarquica SAT. Incluye cuentas de sistema
  pre-pobladas y permite agregar cuentas personalizadas.
  - `code`: Codigo contable (ej. "102", "208.01")
  - `sat_group_code`: Codigo agrupador SAT para Anexo 24
  - `name`: Nombre de la cuenta
  - `account_type`: activo | pasivo | capital | ingreso | gasto | costo
  - `parent_code`: Codigo de cuenta padre para jerarquia
  - `level`: Nivel jerarquico (1=clase, 2=grupo, 3=cuenta, 4=subcuenta)
  - `is_system`: True = cuenta del sistema, no eliminable
  - `is_active`: Si la cuenta esta activa

  ### accounting_entries
  Encabezado de cada poliza contable.
  - `entry_number`: Numero consecutivo de poliza (ej. "I-2026-001")
  - `entry_type`: ingreso | egreso | diario
  - `entry_date`: Fecha contable de la poliza
  - `period_year` / `period_month`: Periodo fiscal
  - `description`: Descripcion de la poliza
  - `source_type`: booking | payout | cancellation | manual
  - `source_id`: UUID del registro origen (booking_id, payout_id, etc.)
  - `is_closing`: Si es poliza de cierre de periodo
  - `is_posted`: Si la poliza esta confirmada/cerrada

  ### accounting_entry_lines
  Partidas individuales de cada poliza (debe sumar cero debito-credito).
  - `entry_id`: FK a accounting_entries
  - `account_code`: Codigo de cuenta de chart_of_accounts
  - `description`: Descripcion de la partida
  - `debit`: Monto debito
  - `credit`: Monto credito
  - `cfdi_uuid`: UUID fiscal del CFDI relacionado (para Anexo 24 polizas)

  ## Seguridad
  - RLS habilitado en todas las tablas
  - Solo admin y accountant pueden ver/modificar
  - chart_of_accounts tiene lectura publica para reportes internos del sistema

  ## Notas
  - El catalogo se pre-pobla con cuentas minimas para RESICO 626
  - Los codigos siguen la estructura del SAT: 1xx Activo, 2xx Pasivo, 3xx Capital, 4xx Ingreso, 5xx Costo, 6xx Gasto
*/
