/*
  # Sistema de Ejecutivos de Cuenta - Parte 1

  ## Descripción
  Crea la infraestructura base del sistema de ejecutivos de cuenta: tablas principales,
  parámetros de comisiones configurables por admin, y extensiones a la tabla agencies.

  ## Nuevas Tablas

  ### account_executives
  Registro de ejecutivos de cuenta. Cada ejecutivo tiene un user_id vinculado a auth.users
  para que pueda iniciar sesión con el rol account_executive.

  ### executive_commission_settings
  Parámetros de comisiones configurables por admin. Solo existe un registro activo.
  - amount_per_approval: monto fijo por agencia aprobada (default $100)
  - amount_per_first_booking: monto fijo cuando agencia publica primer tour Y genera primera reserva pagada (default $100)
  - platform_revenue_percentage: % sobre ingresos de plataforma durante el periodo (default 3%)
  - commission_period_months: cuántos meses dura el periodo de comisión (default 3)

  ### executive_bonus_rules
  Reglas de bonos configurables. Permite agregar bonos por hitos (ej: 100 agencias aprobadas).

  ### agency_leads
  Pipeline de prospectos capturados por ejecutivos. Captura todos los datos del formulario
  de registro de agencias más campos CRM (notas, estado del lead, fecha próximo contacto).

  ### executive_commissions
  Registro de cada comisión generada para un ejecutivo.

  ### executive_bonus_awards
  Registro de bonos ganados por ejecutivos.

  ## Cambios a Tablas Existentes
  - agencies: agrega account_executive_id, registered_by_executive, signed_contract_url,
    approval_period_start, first_tour_published_at, first_paid_booking_at
  - users: agrega account_executive rol (el rol se maneja en la tabla users.role)
  - admin_permissions: agrega can_manage_executives

  ## Seguridad
  - RLS habilitado en todas las tablas nuevas
  - Ejecutivos solo ven sus propios datos
  - Admins ven todo con permiso can_manage_executives
*/
