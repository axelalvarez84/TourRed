/*
  # Sistema de Coordinadores de Agencia (Perfil Dual)

  ## Descripcion
  Crea el sistema de staff/coordinadores para agencias, permitiendo que usuarios
  con rol "traveler" puedan ser vinculados a una agencia como coordinadores.
  Esto implementa el concepto de "perfil dual": el usuario mantiene su cuenta
  de viajero y ademas tiene acceso al panel de agencia segun los permisos otorgados.

  ## Tablas Nuevas

  ### agency_staff
  - Tabla de vinculacion entre usuarios (coordinadores) y agencias
  - Un usuario puede estar vinculado a distintas agencias en diferentes momentos
  - La desvinculacion no elimina el registro, solo marca is_active = false
  - Campos: id, agency_id, user_id, title (cargo), is_active, linked_at, unlinked_at

  ### agency_staff_permissions
  - Permisos granulares de cada coordinador dentro de su agencia
  - Columnas booleanas por cada seccion/accion del panel de agencia
  - Permisos: can_scan_checkin, can_view_bookings, can_manage_tours,
    can_view_financials, can_view_reports, can_manage_discount_codes,
    can_view_messages, can_manage_destinations

  ## Modificaciones

  ### booking_checkin_tokens
  - Agrega columna scanned_by_staff_id (uuid, FK a agency_staff) para auditoria
  - Permite saber que coordinador especifico realizo cada check-in

  ## Seguridad
  - RLS habilitado en ambas tablas nuevas
  - Las agencias solo pueden ver/gestionar su propio staff
  - Los coordinadores solo pueden ver su propia vinculacion activa
