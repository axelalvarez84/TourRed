/*
  # Perfil completo del ejecutivo de cuenta

  ## Descripción
  Agrega columnas de perfil a la tabla account_executives para que cada ejecutivo
  pueda gestionar su información personal, datos fiscales y bancarios desde su portal.

  ## Nuevas columnas en account_executives
  - `profile_photo_url` — URL de la foto de perfil (storage privado)
  - `tax_name`          — Nombre o razón social para efectos fiscales
  - `tax_rfc`           — RFC del ejecutivo
  - `tax_address`       — Domicilio fiscal
  - `tax_zip`           — Código postal fiscal
  - `bank_beneficiary`  — Nombre del titular de la cuenta bancaria
  - `bank_name`         — Nombre del banco (ej. BBVA, Santander)
  - `bank_account_number` — Número de cuenta
  - `bank_clabe`        — CLABE interbancaria (18 dígitos)

  ## Storage
  - Bucket privado `executive-avatars` para fotos de perfil

  ## Seguridad
  - RLS INSERT/SELECT/UPDATE en storage.objects para el bucket executive-avatars
  - Política UPDATE en account_executives para que el ejecutivo edite su propio registro
*/
