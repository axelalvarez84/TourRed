/*
  # Sistema de Tours Receptivos - Parte 1: Tablas y Estructura Base

  ## Descripción
  Agrega soporte para tours de tipo "receptivo" que operan con horarios recurrentes
  y disponibilidad dinámica por fecha/hora, en contraste con las "excursiones" 
  (comportamiento actual) que tienen fechas fijas.

  ## Cambios Principales

  ### 1. Tipo de Tour
  - Agrega columna `tour_type` con enum 'excursion' | 'receptivo'
  - Todos los tours existentes se migran a 'excursion' automáticamente
  - `receptivo_modality` enum 'compartido' | 'privado' (informativo para viajero)

  ### 2. Campos Operativos para Tours Receptivos
  - `operating_days` integer[] - días de semana activos [0-6]
  - `operating_months` integer[] - meses activos [1-12]
  - `min_advance_booking_hours` - horas mínimas de anticipación
  - `max_advance_booking_days` - ventana máxima de reserva en días
  - `slot_duration_days` - duración en días de cada salida
  - `max_daily_slots` - salidas máximas por día
  - `default_slot_capacity` - cupos por defecto por slot
  - `min_travelers_required` - mínimo para confirmar salida
  - `min_travelers_confirmation_hours` - horas antes de evaluar mínimo

  ### 3. Política de Cancelación
  - `cancellation_policy` enum flexible | moderada | estricta | no_reembolsable
  - `cancellation_hours_limit` - horas antes de salida para cancelar
  - `cancellation_refund_percentage` - porcentaje de reembolso

  ### 4. Nueva Tabla `tour_schedules`
  - Horarios recurrentes de salida para tours receptivos
  - Múltiples horarios por tour (mañana, tarde, etc.)
  - Con vigencia, días activos y capacidad específica

  ### 5. Nueva Tabla `tour_slots`
  - Instancias concretas de salida (fecha + hora específica)
  - Tabla central del sistema receptivo
  - Control de cupos en tiempo real

  ### 6. Nueva Tabla `tour_slot_blackouts`
  - Fechas bloqueadas para tours receptivos
  - Soporte de bloqueo parcial por horario

  ## Seguridad
  - RLS habilitado en todas las tablas nuevas
  - Agencias solo acceden a sus propios registros
  - Viajeros solo lectura de slots activos
*/
