/*
  # Corrección completa de funciones y triggers

  1. Nuevas Funciones
    - Corrige todas las funciones relacionadas con notificaciones
    - Corrige todas las funciones relacionadas con mensajería
    - Implementa correctamente los triggers necesarios
  
  2. Seguridad
    - Todas las funciones utilizan SECURITY DEFINER para ejecutarse con los permisos adecuados
    
  3. Cambios
    - Elimina funciones existentes antes de recrearlas para evitar conflictos
    - Verifica la existencia de triggers antes de crearlos
*/

-- Primero eliminamos todas las funciones que vamos a recrear
...
DROP FUNCTION IF EXISTS handle_booking_approval_notification();
DROP FUNCTION IF EXISTS create_user_notification(uuid, notification_type, text, text, jsonb, timestamptz);
...
-- Función para manejar notificaciones de aprobación de reservas
...
-- Función para obtener conversaciones de usuario
...
-- Función para crear una conversación con participantes
...
-- Función para actualizar la marca de tiempo del último mensaje
...
-- Función para marcar un mensaje como editado
...
-- Crear o actualizar los triggers necesarios
DO $$
BEGIN
  -- Trigger para actualizar el último mensaje de una conversación
  IF NOT EXISTS (
  ...
  -- Trigger para marcar un mensaje como editado
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'mark_message_edited_trigger'
  ) THEN
  ...
  -- Trigger para notificaciones de aprobación de reservas
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'booking_approval_notification_trigger'
  ) THEN
...
-- Función para actualizar una reserva y su estado de pago
