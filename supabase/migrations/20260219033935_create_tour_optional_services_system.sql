/*
  # Sistema de Servicios Opcionales para Tours

  ## Descripcion
  Implementa el sistema completo de servicios adicionales opcionales que las agencias pueden
  agregar a sus tours. Los viajeros pueden seleccionar cuantos cupos desean de cada servicio
  al momento de reservar, con control de disponibilidad y politica de reembolso propia.

  ## Nuevas Tablas

  ### tour_optional_services
  - id (uuid, primary key)
  - tour_id (uuid, FK a tours)
  - name (text) — nombre del servicio, ej: "Snorkel", "Boleto Museo"
  - description (text, nullable) — descripcion corta visible al viajero
  - price_per_person (numeric) — precio por persona/cupo
  - max_capacity (integer, nullable) — cupo maximo del servicio; NULL = sin limite
  - is_refundable (boolean) — si el viajero cancela, se reembolsa? default: true
  - is_active (boolean) — visible y disponible para nuevas reservas; default: true
  - display_order (integer) — orden de visualizacion en el formulario; default: 0
  - created_at, updated_at

  ### booking_optional_services
  - id (uuid, primary key)
  - booking_id (uuid, FK a bookings)
  - tour_optional_service_id (uuid, FK a tour_optional_services)
  - quantity (integer) — numero de cupos reservados
  - unit_price (numeric) — precio al momento de la reserva (snapshot)
  - subtotal (numeric) — quantity * unit_price
  - is_cancelled (boolean) — true si fue cancelado (individual o con la reserva)
  - cancelled_at (timestamptz, nullable)
  - refund_amount (numeric) — cuanto se reembolso al cancelar; 0 si no reembolsable
  - cancelled_by_agency (boolean) — true si la cancelacion fue iniciada por la agencia
  - created_at, updated_at

  ## Seguridad
  - RLS habilitado en ambas tablas
  - tour_optional_services: lectura publica (para mostrar en catalogo), escritura solo agencia duena
  - booking_optional_services: lectura por el viajero dueno y la agencia, escritura solo sistema
*/
