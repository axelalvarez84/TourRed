/*
  # Create ToursRed+ Memberships System
  ...
  1. New Tables
    - `memberships`
      ...
      - `cancel_at_period_end` (boolean) - Auto-renew disabled
      - `cancelled_at` (timestamptz, nullable) - When subscription was cancelled
      - `service_fee_exemption_used` (decimal) - Amount of exemption used this month
      - `service_fee_exemption_reset_date` (timestamptz) - When to reset the $500 counter
  ...
  2. Security
    ...
    - Users can update their own service_fee_exemption_used
    - Admins can view all memberships
    - Service role can insert/update memberships (for Stripe webhooks)

  3. Indexes
    - Index on user_id for faster lookups
    - Index on stripe_subscription_id for webhook processing
    - Index on status for filtering active memberships

  4. Functions
    - Function to check if user has active membership
    - Function to get available service fee exemption amount
    - Function to reset monthly service fee exemption
*/
