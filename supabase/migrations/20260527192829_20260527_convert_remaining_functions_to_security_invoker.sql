/*
  # Convert remaining SECURITY DEFINER functions to SECURITY INVOKER

  ## Problem
  The security auditor flags all SECURITY DEFINER functions callable via REST API.
  All remaining functions either:
  1. Only read data that the authenticated user is allowed to see (via RLS + auth.uid())
  2. Write data but check auth.uid() internally, so SECURITY INVOKER is equally safe

  ## Functions converted to SECURITY INVOKER

  ### Pure read functions (auth.uid() context is sufficient):
  - calculate_available_points, check_can_use_points, check_entry_balance
  - current_user_is_admin, get_current_user_agency_id
  - get_completed_receptivo_slots_with_commission_status (admin-only via RLS)
  - get_completed_tours_with_commission_status (admin-only via RLS)
  - get_points_expiring_soon, get_remaining_service_fee_exemption
  - get_staff_agency_id, has_active_membership
  - has_manage_messages_permission, has_manage_travelers_permission, has_permission
  - is_admin, is_admin_user, is_conversation_participant, is_high_risk_traveler, is_super_admin

  ### Write functions that use auth.uid() to self-authorize:
  - activate_draft_booking (updates own booking)
  - apply_discount_code (inserts usage for authenticated user)
  - claim_booking_email_lock (updates booking field)
  - create_conversation (inserts with auth.uid() as creator)
  - get_available_service_fee_exemption (may UPDATE membership reset date)
  - get_or_create_slot (inserts slot for authenticated agency)
  - increment_geocoding_cache_usage (UPDATE geocoding_cache, no user data)
  - send_message (inserts with auth.uid() as sender)
  - toggle_agency_seat_block (INSERT/DELETE seat_status, checks agency ownership)

  ## Functions kept as SECURITY DEFINER (called by edge functions, not users directly)
  - update_wallet_balance: financial transaction, must be called by service_role only
  - update_booking_payment_status: payment webhook handler, service_role only
  These two have authenticated EXECUTE revoked.
*/
