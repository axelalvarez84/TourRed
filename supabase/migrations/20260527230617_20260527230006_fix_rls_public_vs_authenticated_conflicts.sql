/*
  # Fix Multiple Permissive Policies — public vs authenticated conflicts

  ## Problema
  El Performance Advisor detecta conflicto cuando existen dos políticas PERMISSIVE
  para la misma acción y el rol {authenticated} está cubierto por ambas:
  - {public} SELECT + {authenticated} SELECT  → authenticated recibe dos políticas SELECT
  - {public} SELECT + {authenticated} ALL     → ALL cubre SELECT, mismo conflicto
  - {anon,authenticated} SELECT + {authenticated} SELECT → authenticated aparece en ambos

  ## Solución
  Fusionar las políticas conflictivas:
  - Para acceso público sin autenticación: mantener TO anon con la condición pública
  - Para usuarios autenticados: una sola política SELECT con condiciones OR que absorba
    tanto la condición pública como la condición privilegiada
  - Para tablas con ALL + SELECT public: descomponer ALL en acciones, el SELECT
    absorbe la condición pública

  ## Tablas modificadas
  1.  agency_reviews     — {public} SELECT + {authenticated} SELECT
  2.  departure_points   — {public} SELECT + {authenticated} SELECT
  3.  reviews            — {public} SELECT + {authenticated} SELECT
  4.  terms_versions     — {public} SELECT + {authenticated} SELECT
  5.  tour_categories    — {public} SELECT + {authenticated} SELECT
  6.  users              — {public} SELECT (USING true) + {authenticated} SELECT
  7.  destination_images — {public} SELECT + {authenticated} ALL
  8.  tour_destinations  — {public} SELECT + {authenticated} ALL
  9.  tours              — {public} SELECT + {authenticated} ALL
  10. tour_promotions    — {anon,authenticated} SELECT + {authenticated} SELECT
*/
