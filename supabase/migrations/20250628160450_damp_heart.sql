...
COMMENT ON COLUMN destinations.main_image_url IS 'URL to the main image (alternative to base64)';
...
-- Create or replace the image size validation function with robust error handling
CREATE OR REPLACE FUNCTION validate_image_size()
RETURNS TRIGGER AS $$
BEGIN
  -- For destinations table
  IF TG_TABLE_NAME = 'destinations' THEN
    -- Only validate if main_image_base64 is provided
    IF NEW.main_image_base64 IS NOT NULL THEN
      -- Calculate size if not provided
      IF NEW.main_image_size IS NULL OR NEW.main_image_size = 0 THEN
  ...
  -- For destination_images table
  IF TG_TABLE_NAME = 'destination_images' THEN
    -- Only validate if image_base64 is provided
    IF NEW.image_base64 IS NOT NULL THEN
      -- Calculate size if not provided
      IF NEW.image_size IS NULL OR NEW.image_size = 0 THEN
...
-- Create new trigger for destination_images table
CREATE TRIGGER validate_destination_images_size
  BEFORE INSERT OR UPDATE ON destination_images
  FOR EACH ROW
  EXECUTE FUNCTION validate_image_size();

-- Ensure destination_images table has all required columns
...
-- Add comments to the columns for documentation
COMMENT ON COLUMN destination_images.image_base64 IS 'Image in base64 format (maximum 3MB)';
COMMENT ON COLUMN destination_images.image_type IS 'MIME type of the image (image/jpeg, image/png, etc.)';
COMMENT ON COLUMN destination_images.image_size IS 'Size of the image base64 in bytes';
