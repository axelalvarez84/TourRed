/*
  # Create Simple Departure Points Catalog System
  
  ## Overview
  This migration replaces the complex geolocation system with a simple catalog-based approach
  for managing departure points. Agencies can create and reuse departure points without
  complex geocoding or external API dependencies.
  
  ## Changes Made
  
  ### 1. Cleanup - Remove Old Tables
  - Drops old geolocation tables: departure_locations, tour_departure_locations, geocoding_cache
  - Drops transport/OSM tables: cities, transport_systems, osm_sync_logs, featured_pois
  - Removes associated indexes, triggers, and functions
  
  ### 2. New Tables Created
  
  #### `departure_points`
  - Simple catalog of departure locations (name, city, municipality)
  - Optional Google Maps URL for exact location
  - Tracks who created each point
  - Support for active/inactive status
  
  #### `tour_departure_points`
  - Junction table linking tours to 1-4 departure points
  - Includes display_order (1-4) for ordering points
  - Constraints ensure minimum 1 and maximum 4 points per tour
  
  ### 3. Security (RLS Policies)
  - Public read access for active departure points
  - Agencies and admins can create new points
  - Admins can edit/deactivate points
  - Proper RLS on junction table for tour management
  
  ### 4. Indexes
  - Efficient querying by name, city, municipality
  - Usage tracking for popularity statistics
