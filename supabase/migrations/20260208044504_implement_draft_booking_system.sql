/*
  # Implement Draft Booking System

  ## Overview
  Defers spot blocking until the user actually proceeds to payment (or agency approves for manual tours).
  Bookings are initially created with status 'draft' which does NOT count toward tour availability.

  ## Changes

  ### 1. Booking Status Constraint
  - Add 'draft' as a valid booking status value

  ### 2. Tour Availability Function
  - Updated to only count spots for:
    - status = 'confirmed'
    - status = 'pending' AND approval_status = 'approved'
  - Draft bookings and pending-approval bookings do NOT block spots

  ### 3. Notification Trigger
  - Updated to skip notifications for 'draft' bookings

  ### 4. New Function: activate_draft_booking
  - Atomically checks availability and transitions booking from 'draft' to 'pending'
  - Prevents race conditions when multiple users try to book the last spots

  ### 5. New Function: cleanup_abandoned_draft_bookings
  - Deletes draft bookings older than 2 hours
  - Also deletes associated booking_travelers records

  ### 6. Cron Job
  - Runs cleanup every hour

  ## Security
  - activate_draft_booking uses SECURITY DEFINER for atomic availability check
  - cleanup function uses SECURITY DEFINER for system-level cleanup
*/
