/*
  # Create CFDI Billing System

  ## Summary
  Implements a complete PAC-agnostic CFDI invoicing system for the platform.
  Designed with an abstraction layer so the underlying PAC (FacturAPI, SW Sapien, etc.)
  can be swapped without modifying business logic.

  ## New Tables

  1. `cfdi_invoices`
     - Stores all CFDI documents issued (both booking receipts and commission invoices)
     - `id` (uuid, PK)
     - `invoice_type` ('booking' | 'commission') - purpose of the CFDI
     - `booking_id` (uuid, FK) - linked booking if type = booking
     - `payout_id` (uuid, FK) - linked payout if type = commission
     - `agency_id` (uuid, FK) - the agency involved
     - `pac_provider` (text) - which PAC generated this (facturapi, sw_sapien, etc.)
     - `pac_invoice_id` (text) - the PAC's internal ID for this document
     - `uuid_fiscal` (text) - SAT UUID (folio fiscal), unique per document
     - `folio` (text) - company folio (e.g. A-0001)
     - `serie` (text) - CFDI series (e.g. A)
     - `receptor_rfc` (text) - RFC del receptor
     - `receptor_razon_social` (text) - Name of the receptor
     - `receptor_regimen_fiscal` (text) - Fiscal regimen code
     - `receptor_uso_cfdi` (text) - CFDI use code (G01, G03, etc.)
     - `receptor_codigo_postal` (text) - Fiscal postal code of receptor
     - `subtotal` (numeric) - Amount before tax
     - `iva_amount` (numeric) - IVA 16% amount
     - `total` (numeric) - Total amount
     - `currency` (text, default 'MXN')
     - `status` (text) - 'pending', 'stamped', 'cancelled', 'error'
     - `xml_url` (text) - Public URL for the XML file
     - `pdf_url` (text) - Public URL for the PDF file
     - `stamped_at` (timestamptz) - When the SAT stamped the document
     - `error_message` (text) - Last error if status = error
     - `retry_count` (int) - Number of retry attempts
     - `email_sent` (boolean) - Whether recipient received the email
     - `created_at` (timestamptz)
     - `updated_at` (timestamptz)

  2. `cfdi_cancellation_requests`
     - Tracks cancellation requests for previously stamped CFDIs
     - `id` (uuid, PK)
     - `cfdi_invoice_id` (uuid, FK to cfdi_invoices)
     - `motivo` (text) - SAT cancellation reason code (01, 02, 03, 04)
     - `uuid_sustitucion` (text, nullable) - UUID of replacement CFDI if motivo = 01
     - `status` (text) - 'pending', 'accepted', 'rejected'
     - `pac_cancellation_id` (text) - PAC's cancellation request ID
  ...
  3. `users` - Added fiscal data fields for travelers:
     - `rfc` (text, nullable)
     - `razon_social` (text, nullable)
     - `regimen_fiscal` (text, nullable) - SAT fiscal regimen code
     - `uso_cfdi` (text, nullable) - Default CFDI usage code
     - `codigo_postal_fiscal` (text, nullable) - Fiscal ZIP code

  4. `agencies` - Added fiscal data fields:
     - `rfc` (text, nullable)
     - `razon_social` (text, nullable)
     - `regimen_fiscal` (text, nullable)

  5. `platform_settings` - Added CFDI/PAC configuration:
     - `pac_provider` (text) - Active PAC: 'facturapi', 'sw_sapien', 'none'
     - `pac_api_key_encrypted` (text) - Encrypted PAC API key (stored as text, encrypted at app level)
     - `pac_organization_id` (text) - PAC organization/issuer ID
     - `cfdi_serie_booking` (text) - CFDI series for booking invoices (default 'A')
     - `cfdi_serie_commission` (text) - CFDI series for commission invoices (default 'B')
     - `pac_sandbox_mode` (boolean) - Whether PAC is in test/sandbox mode
     - `pac_issuer_rfc` (text) - ToursRed's RFC as CFDI issuer
     - `pac_issuer_razon_social` (text) - ToursRed's legal name
     - `pac_issuer_regimen_fiscal` (text) - ToursRed's fiscal regimen code

  ## Security
  - RLS enabled on all new tables
  - Admins can view and manage all CFDI records
  - Agencies can view their own CFDI records
  - Travelers can view their own CFDI records
  - Only service role can insert/update CFDI records (done via edge functions)
*/
