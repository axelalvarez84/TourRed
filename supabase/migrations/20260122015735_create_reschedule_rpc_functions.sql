/*
  # Funciones RPC para Sistema de Reagendamiento

  ## Descripción
  Funciones de base de datos para soportar el sistema de reagendamiento de tours.

  ## Funciones Creadas

  ### `get_pending_reschedule_for_booking`
  Obtiene información del reagendamiento pendiente para una reserva específica.
  Retorna datos del tour_reschedule y booking_reschedule_response combinados.
  
  Parámetros:
  - p_booking_id: UUID de la reserva
  
  Retorna:
  - JSON con información completa del reagendamiento o NULL si no hay pendiente

  ### `get_reschedule_summary_for_tour`
  Obtiene resumen de respuestas a un reagendamiento de tour.
  Útil para que las agencias vean el progreso de respuestas.
  
  Parámetros:
  - p_tour_reschedule_id: UUID del reagendamiento
  
  Retorna:
  - JSON con contadores de respuestas (pending, accepted, rejected, auto_accepted)

  ### `auto_accept_expired_reschedules`
  Auto-acepta reagendamientos que pasaron la fecha límite sin respuesta.
  Se ejecutará por un cron job o manualmente.
  
  Retorna:
  - Número de respuestas auto-aceptadas
*/
