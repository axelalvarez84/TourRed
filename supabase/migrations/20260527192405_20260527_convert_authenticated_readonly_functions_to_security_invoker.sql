/*
  # Convert authenticated read-only functions to SECURITY INVOKER

  ## Problem
  Several SECURITY DEFINER functions accessible by the authenticated role are flagged
  by the auditor. Functions that only read data and use auth.uid() internally
  can safely use SECURITY INVOKER since the caller already has their identity.

  ## Functions converted to SECURITY INVOKER
  These only read data and use auth.uid() — SECURITY INVOKER is safe and correct:
  - _get_user_conversations_internal (internal, also revoke direct REST access)
  - get_accounting_sync_stats, get_account_balances_full, get_balance_sheet
  - get_income_statement, get_trial_balance
  - get_agency_financial_summary, get_agency_owner_id, get_agency_penalty_summary
  - get_agency_request_ids, get_agency_tours
  - get_alternative_slots_for_reschedule, get_booking_payment_details
  - get_cfdi_stats, get_completed_receptivo_slots_with_commission_status
  - get_completed_tours_with_commission_status, get_confirmed_spots_in_reschedule
  - get_conversation_messages, get_discount_code_details
  - get_next_available_slot, get_pending_reschedule_for_booking
  - get_reschedule_summary_for_tour, get_staff_with_permissions
  - get_tour_confirmed_attendees, get_tour_slots_by_range
  - get_unread_notifications_count (both overloads)
  - get_user_conversations, get_user_notifications
  - is_tour_ready_for_payout
  - mark_all_notifications_as_read, mark_conversation_read
  - mark_messages_as_read, mark_notification_as_read, mark_notifications_as_read

  ## Functions kept as SECURITY DEFINER
  These write to sensitive tables and need elevated privileges:
  - activate_draft_booking (UPDATE bookings)
  - apply_discount_code (INSERT discount_code_usage)
  - claim_booking_email_lock (UPDATE bookings)
  - create_conversation (INSERT conversations + message_participants)
  - get_or_create_slot (INSERT tour_slots)
  - send_message (INSERT messages)
  - toggle_agency_seat_block (INSERT/DELETE slot_seat_status)
  - update_booking_payment_status (UPDATE bookings)
  - update_wallet_balance (UPDATE toursred_cash_wallets)
  - check_entry_balance (fine as is, accounting internal)
*/
