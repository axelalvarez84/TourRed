/*
  # Fix REVOKE PUBLIC on SECURITY DEFINER functions

  ## Problem
  Previous migration used REVOKE FROM anon (explicit role), but each function still had
  =X/postgres in its ACL, which represents the PUBLIC role (inherited by anon).
  The auditor detects anon can execute SECURITY DEFINER functions via PUBLIC role access.

  ## Solution
  Use dynamic SQL to:
  1. REVOKE EXECUTE FROM PUBLIC on all SECURITY DEFINER functions in public schema
     (except functions that legitimately need anon access)
  2. Re-GRANT EXECUTE TO authenticated for functions that need it
  3. Re-GRANT EXECUTE TO anon, authenticated for public catalog functions

  ## Functions kept accessible to anon (public catalog)
  These functions are needed for unauthenticated users browsing tours:
