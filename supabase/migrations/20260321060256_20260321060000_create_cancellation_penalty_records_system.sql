/*
  # Sistema de Penalizaciones por Cancelacion

  ## Resumen
  Implementa un sistema completo para registrar y gestionar los montos que corresponden
  a la agencia cuando un viajero cancela con politica de penalizacion (50% o sin reembolso).
  Estos montos son conceptualmente diferentes a las comisiones de tours completados:
  son penalizaciones por cancelacion tardia que la plataforma retiene del pago del viajero
  y que le corresponden a la agencia.

  ## Cambios

  ### 1. Nuevo estado en bookings
  - Agrega payment_not_received al CHECK constraint de bookings.status
    para identificar reservas canceladas por falta de pago

  ### 2. Nueva Tabla: cancellation_penalty_records
  - booking_id, agency_id, tour_id: referencias clave
  - cancellation_type: 'full' o 'partial'
  - cancellation_policy_type: '50_percent' o 'no_refund'
  - gross_penalty: monto total retenido al viajero
  - agency_net_amount: lo que le toca a la agencia
  - platform_amount: lo que se queda la plataforma
  - status: 'pending' o 'processed'
  - Campos de pago: method, notes, receipt_url, processed_at

  ### 3. Seguridad (RLS)
  - Agencias: ven sus propios registros
  - Viajeros: ven registros de sus reservas
  - Admins: ven y actualizan todos
  - Service role: insertar y actualizar

  ### 4. Funcion helper: get_agency_penalty_summary
*/
