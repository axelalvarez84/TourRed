/*
  # Sistema de Tours Receptivos - Parte 2: Modificaciones a Bookings, Funciones RPC y Triggers

  ## Descripción
  Agrega las columnas necesarias en bookings para referenciar slots,
  crea funciones RPC para gestión de disponibilidad y triggers para
  mantener contadores en tiempo real.

  ## Cambios

  ### 1. Tabla `bookings` - Nuevas columnas
  - `slot_id` - referencia al slot reservado (tours receptivos)
  - `selected_date` - fecha de salida seleccionada
  - `selected_time` - hora de salida seleccionada

  ### 2. Funciones RPC
  - `get_tour_slots_by_range` - slots en un rango de fechas con disponibilidad
  - `get_or_create_slot` - obtiene o crea un slot para fecha+horario
  - `get_next_available_slot` - próximo slot activo con cupos
  - `auto_generate_slots_for_range` - genera slots automáticamente
  - `get_tour_availability_v2` - versión mejorada con soporte de slots

  ### 3. Triggers
  - `update_slot_booked_count` - mantiene booked_count actualizado
  - `check_min_travelers_reached` - evalúa si se alcanzó el mínimo
*/
