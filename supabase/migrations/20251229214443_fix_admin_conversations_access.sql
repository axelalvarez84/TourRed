...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(COUNT(m.id) FILTER (WHERE m.sender_id != v_target_user_id AND m.created_at > COALESCE(mp_user.last_read_at, '1970-01-01'::timestamptz)), 0)::bigint as unread_count,
  ...
  LEFT JOIN message_participants mp_user ON c.id = mp_user.conversation_id AND mp_user.user_id = v_target_user_id
  LEFT JOIN messages m ON c.id = m.conversation_id
  LEFT JOIN tours t ON c.tour_id = t.id
  LEFT JOIN message_participants mp_all ON c.id = mp_all.conversation_id
  LEFT JOIN LATERAL (
