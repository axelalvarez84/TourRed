/*
  # Create Referral System for ToursRed Points
  
  ## Overview
  Complete referral system where users can invite friends and earn points when their referrals complete their first booking.
  
  ## New Tables
  
  ### `referral_codes`
  - `id` (uuid, primary key)
  - `user_id` (uuid, foreign key to users) - Owner of the referral code
  - `code` (text, unique) - 8-character alphanumeric code (uppercase letters + numbers, excluding ambiguous chars)
  - `is_active` (boolean) - Whether code can be used
  - `successful_referrals_count` (integer) - Count of completed referrals
  - `max_referrals_allowed` (integer) - Maximum number of referrals allowed (default from platform_settings)
  - `created_at` (timestamptz)
  - `updated_at` (timestamptz)
  
  ### `referral_relationships`
  - `id` (uuid, primary key)
  - `referrer_user_id` (uuid, foreign key to users) - User who referred
  - `referred_user_id` (uuid, foreign key to users) - User who was referred
  - `referral_code_used` (text) - The code that was used
  - `status` (text) - 'pending', 'completed', 'cancelled'
  - `referrer_bonus_awarded` (boolean) - Whether referrer received bonus
  - `referred_bonus_awarded` (boolean) - Whether referred received bonus
  - `first_booking_id` (uuid, nullable, foreign key to bookings) - First confirmed booking
  - `created_at` (timestamptz) - When referral relationship was created
  - `completed_at` (timestamptz, nullable) - When first booking was completed
  - `is_suspicious` (boolean) - Flag for fraud detection
  
  ### `referral_bonuses`
  - `id` (uuid, primary key)
  - `referral_relationship_id` (uuid, foreign key to referral_relationships)
  - `user_id` (uuid, foreign key to users) - Who received the bonus
  - `points_amount` (integer) - Amount of points awarded
  - `status` (text) - 'pending', 'awarded', 'expired'
  - `awarded_at` (timestamptz, nullable)
  - `reason` (text) - Description of why bonus was awarded
  - `created_at` (timestamptz)
  
  ### `referral_fraud_logs`
  - `id` (uuid, primary key)
  - `referral_relationship_id` (uuid, foreign key)
  - `fraud_type` (text) - Type of suspicious activity detected
  ...
  ### `users`
  - Add `referred_by_user_id` (uuid, nullable, foreign key to users)
  - Add `referral_code_used` (text, nullable)
  
  ### `bookings`
  - Add `referral_bonus_awarded` (boolean, default false)
  
  ### `platform_settings`
  - Add `default_max_referrals_per_user` (integer, default 10)
  - Add `referral_bonus_points` (integer, default 5000)
  - Add `referral_program_enabled` (boolean, default true)
  
  ## Functions
  
  1. `generate_unique_referral_code()` - Generates unique 8-char code
  2. `create_referral_code_on_signup()` - Trigger to auto-create code for new travelers
  3. `award_referral_bonus()` - Awards points when referral completes first booking
  4. `check_referral_bonus_eligibility()` - Trigger that checks if bonus should be awarded
  
  ## Security
  - Enable RLS on all new tables
  - Users can view their own referral codes and relationships
  - Users can view their received bonuses
  - Admins can view and manage all referral data
  - Public can validate referral codes (security definer function)
*/
