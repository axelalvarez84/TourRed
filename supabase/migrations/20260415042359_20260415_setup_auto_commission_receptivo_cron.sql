/*
  # Cron Job: Auto-crear commission_records para slots receptivos completados

  ## Descripcion
  Configura un job diario que a medianoche busca todos los slots de tours receptivos
  cuya fecha ya paso (slot_date < CURRENT_DATE), y para cada uno crea automaticamente
  los commission_records de las reservas confirmadas y pagadas que aun no los tengan.

  Esto garantiza que:
  - Ningun slot receptivo completado quede sin sus comisiones generadas
  - El admin solo necesita revisar y procesar el pago, no crear comisiones manualmente
  - La vista "Por Slot Completado" siempre tenga datos actualizados

  ## Funcion auxiliar: auto_create_receptivo_slot_commissions
  Itera sobre todos los slots receptivos completados y llama
  create_commission_records_for_receptivo_slot para cada uno.

  ## Cron Schedule
  Ejecuta diariamente a las 02:00 UTC (horario de menor actividad)
*/
