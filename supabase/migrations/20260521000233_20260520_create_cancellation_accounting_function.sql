/*
  # Contabilidad de Cancelaciones — Funcion y Cuentas

  ## Descripcion
  Implementa la funcion `create_accounting_entry_for_cancellation` que estaba documentada
  pero nunca fue creada. Genera asientos contables correctos para todos los tipos de
  cancelacion: viajero cancela (total/parcial), agencia cancela tour, agencia cancela reserva.

  ## Modelo contable de cancelaciones

  ### Cancelacion con reembolso total (100%) — viajero o agencia cancela:
    Dr. 208 Anticipos de clientes (reverso del pasivo)  = deposit_amount
    Cr. 101.01 / 102.01 Bancos (sale el reembolso)      = refund_amount
    — Si service_charge no se reembolsa, se reconoce como ingreso ganado:
    Cr. 402 Ingresos por cargo de servicio              = service_charge (ya registrado en booking)

  ### Cancelacion con 50% de penalizacion — viajero cancela 7-14 dias antes:
    Dr. 208 Anticipos de clientes                       = deposit completo del viajero
    Cr. 101.01 / 102.01 Bancos                          = 50% (sale el reembolso al viajero)
    Cr. 201 Proveedores (CxP agencias)                  = 70% del 50% retenido (adeudo a agencia)
    Cr. 402 Ingresos por penalizaciones                 = 30% del 50% retenido (ingreso ToursRed)

  ### Cancelacion sin reembolso (no_refund) — viajero cancela <7 dias:
    Dr. 208 Anticipos de clientes                       = deposit completo
    Cr. 201 Proveedores (CxP agencias)                  = (1 - commission_rate) * deposit
    Cr. 402 Ingresos por penalizaciones                 = commission_rate * deposit

  ### Cancelacion parcial (misma logica pero sobre el monto parcial proporcional)

  ## Nuevas cuentas contables
    - 402.01 Ingresos por cargo de servicio (ya existe como 402)
    - 402.02 Ingresos por penalizaciones de cancelacion (nueva subcuenta)
    - 606.01 Reembolsos a viajeros (subcuenta de 606)

  ## Notas de seguridad
    - SECURITY DEFINER para que Edge Functions la puedan llamar con service_role
    - Verifica duplicados con source_type + source_id para idempotencia
    - No genera poliza si el booking no esta cancelado (proteccion adicional)
*/
