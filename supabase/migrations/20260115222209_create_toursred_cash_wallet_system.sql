/*
  # Create ToursRed Cash Wallet System
  ...
  1. New Tables
    ...
    - `toursred_cash_transactions`
      - `id` (uuid, primary key)
      - `wallet_id` (uuid, foreign key to toursred_cash_wallets)
      - `user_id` (uuid, foreign key to users)
      - `amount` (decimal, positive for credits, negative for debits)
      - `balance_after` (decimal, balance after transaction for audit trail)
      - `type` (text, check constraint: credit, debit, refund, promotion, gift_card, adjustment)
      - `description` (text)
      - `reference_id` (uuid, nullable, for linking to bookings, gift cards, etc)
      - `reference_type` (text, nullable, e.g., 'booking', 'gift_card', 'promotion')
      - `created_at` (timestamptz)
  ...
  2. Security
    ...
    - Only system/admin can modify wallets and create transactions
