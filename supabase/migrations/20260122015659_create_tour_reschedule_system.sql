/*
  # Sistema de Reagendamiento de Tours

  ## Descripción
  Sistema completo para que las agencias puedan reagendar tours por motivos como clima, logística, etc.
  Los viajeros afectados pueden aceptar la nueva fecha o rechazar y recibir reembolso 100% sin penalización.

  ## Nuevas Tablas
  
  ### `tour_reschedules`
  Almacena información de cada reagendamiento de tour realizado por una agencia.
  - `id` - UUID primary key
  - `tour_id` - Referencia al tour reagendado
  - `agency_id` - Agencia que realizó el reagendamiento
  - `original_start_date` - Fecha de inicio original
  - `original_end_date` - Fecha de fin original
  - `new_start_date` - Nueva fecha de inicio
  - `new_end_date` - Nueva fecha de fin
  - `reason` - Motivo del reagendamiento (requerido)
  - `created_by` - Usuario de la agencia que creó el reagendamiento
  - `affected_bookings_count` - Contador de reservas afectadas
  - `status` - Estado: pending_responses, completed, cancelled
  - `response_deadline` - Fecha límite para que viajeros respondan
  - `created_at` - Timestamp de creación

  ### `booking_reschedule_responses`
  Rastrea las respuestas individuales de cada viajero a un reagendamiento.
  - `id` - UUID primary key
  - `tour_reschedule_id` - Referencia al reagendamiento
  - `booking_id` - Reserva afectada
  - `user_id` - Viajero que debe responder
  - `response` - Estado: pending, accepted, rejected, auto_accepted
  - `responded_at` - Timestamp de respuesta
  - `refund_processed` - Si se procesó reembolso (para rechazos)
  - `refund_transaction_id` - Referencia a la transacción de reembolso
  - `notification_sent` - Si se envió notificación in-app
  - `email_sent` - Si se envió email
  - `reminder_sent_at` - Timestamp del último recordatorio enviado
  - `created_at` - Timestamp de creación

  ## Modificaciones a Tablas Existentes
  
  ### `bookings`
  - `has_pending_reschedule` - Boolean para indicar reagendamiento pendiente
  - `reschedule_response` - Respuesta del viajero: accepted, rejected, auto_accepted
  - `reschedule_responded_at` - Timestamp de la respuesta
  - `original_booking_date` - Fecha original antes del reagendamiento

  ### `notifications` (enum type)
  - Agregar 'tour_rescheduled' al enum notification_type

  ## Seguridad (RLS)
  - Agencias pueden ver/crear sus propios reagendamientos
  - Viajeros pueden ver reagendamientos que afectan sus reservas
  - Viajeros solo pueden responder a sus propias reservas
  - Solo usuarios autenticados pueden acceder

  ## Índices
  - Índice en tour_reschedules(tour_id) para búsquedas por tour
  - Índice en booking_reschedule_responses(booking_id) para búsquedas por reserva
  - Índice en bookings(has_pending_reschedule) para filtros rápidos
*/
