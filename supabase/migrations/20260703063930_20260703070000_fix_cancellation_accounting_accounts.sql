-- Corrección 1: cuenta para penalización a agencia: 201 → 218 (Otros Pasivos Circulantes)
-- Corrección 2: cuenta residual negativo (reverso seguro): 606 → 401.02

-- Asegurar que la cuenta 218 existe
INSERT INTO chart_of_accounts (code, sat_group_code, name, account_type, parent_code, level, nature, is_system, description)
VALUES ('218', '205-01', 'Otros pasivos circulantes', 'pasivo', '200', 3, 'acreedora', false,
        'Pasivos circulantes varios — incluye penalizaciones por cancelación por pagar a agencias')
...
BEGIN
  ...
  -- Línea 1: Reverso anticipo (208 Anticipos de clientes)
  IF v_deposit > 0 THEN
  ...
  -- Línea 2: Reembolso al viajero vía ToursRed Cash (102.01 Bancos)
  ...
  -- Línea 3: Penalización parte agencia (218 Otros Pasivos Circulantes — por pagar a agencia)
  IF v_amount_to_agency > 0 THEN
    INSERT INTO accounting_entry_lines
      (entry_id, line_number, account_code, description, debit, credit, cfdi_uuid)
    VALUES (
      v_entry_id, 3, '218',
      'Penalizacion por cancelacion — por pagar a agencia ' || COALESCE(v_tour_name, ''),
      0, v_amount_to_agency, v_cfdi_uuid
    );
  END IF;

  -- Línea 4: Ingreso plataforma por penalización (402.02)
  ...
  BEGIN
    ...
    IF ABS(v_residual) > 0.001 THEN
      IF v_residual > 0 THEN
        -- Débito mayor: crédito a ingresos por servicios no reembolsables
        INSERT INTO accounting_entry_lines
          (entry_id, line_number, account_code, description, debit, credit, cfdi_uuid)
        VALUES (
          v_entry_id, 5, '402',
      ...
      ELSE
        -- Crédito mayor (ej. reverso de seguro de viaje): debitar 401.02
        INSERT INTO accounting_entry_lines
          (entry_id, line_number, account_code, description, debit, credit, cfdi_uuid)
        VALUES (
          v_entry_id, 5, '401.02',
          'Reverso ingreso seguro cancelado — ' || COALESCE(v_booking_code, ''),
