/*
  # Create Geolocation System with PostGIS
  
  1. Enable PostGIS Extension
    - Enables spatial data types and functions for geographic queries
  
  2. New Tables
    - `departure_locations`
      - Stores normalized departure locations with geographic coordinates
      - Uses PostGIS GEOGRAPHY type for accurate distance calculations
      - Supports multiple names/aliases per location
      - Includes full Mapbox address details
    
    - `tour_departure_locations`
      - Junction table linking tours to their departure locations
      - Supports multiple departure points per tour
      - Includes meeting time and instructions per location
    
    - `geocoding_cache`
      - Caches Mapbox geocoding API responses
      - Reduces API calls and improves performance
      - 90-day expiration for cache entries
  
  3. Indexes
    - Spatial index (GIST) on location geography for fast proximity searches
    - GIN index on aliases array for text search
    - B-tree indexes on foreign keys and common query fields
  
  4. Security
    - Enable RLS on all tables
    - Public read access for departure_locations
    - Authenticated users can insert geocoding cache
    - Agencies can manage their tour departure locations
