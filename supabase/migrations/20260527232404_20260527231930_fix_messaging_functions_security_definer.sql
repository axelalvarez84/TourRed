/*
  # Fix stack depth limit exceeded in messaging

  ## Causa raíz
  is_admin_user(), is_conversation_participant(), get_user_conversations() y
  _get_user_conversations_internal() carecen de SECURITY DEFINER. Al ser invocadas
  desde políticas RLS, sus SELECT internos activan a su vez las políticas de las tablas
  que consultan (users, message_participants, conversations), generando recursión hasta
  agotar el stack de PostgreSQL.

  ## Solución
  Agregar SECURITY DEFINER + SET search_path = public a todas las funciones del sistema
  de mensajería que consultan tablas con RLS activo.

  ## Sin nuevos warnings
  - SECURITY DEFINER + SET search_path = public en todas las funciones
  - REVOKE EXECUTE FROM PUBLIC + GRANT solo a authenticated
  - DROP + RECREATE necesario para get_user_conversations (tiene tipo TABLE en retorno)
*/
