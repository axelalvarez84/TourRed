/*
  # Sistema de Comisiones para Tours Receptivos Completados

  ## Descripcion
  Los tours receptivos no tienen end_date fija, sino que operan por slots con fecha especifica
  (bookings.selected_date / tour_slots.slot_date). Un slot se considera completado cuando
  su fecha ya paso y no fue cancelado.

  Este sistema implementa:

  1. Funcion `get_completed_receptivo_slots_with_commission_status`
     - Agrupa las reservas de tours receptivos por slot (selected_date + selected_time + tour_id)
     - Solo incluye slots cuya fecha ya paso (selected_date < CURRENT_DATE)
     - Calcula estado de comisiones por slot
     - Equivalente a get_completed_tours_with_commission_status pero para receptivos

  2. Funcion `create_commission_records_for_receptivo_slot`
     - Crea commission_records para todas las reservas confirmadas de un slot completado
     - Recibe slot_id o combinacion tour_id + selected_date + selected_time
     - Usa selected_date como tour_end_date en el commission_record

  ## Tablas Afectadas
  - commission_records: lectura y escritura
  - bookings: lectura (filtra por selected_date, slot_id, status)
  - tour_slots: lectura
  - tours: lectura (filtra por tour_type = 'receptivo')
  - agencies: lectura

  ## Seguridad
  - Ambas funciones usan SECURITY DEFINER con search_path = public
  - Solo admins invocan estas funciones desde la UI
*/
