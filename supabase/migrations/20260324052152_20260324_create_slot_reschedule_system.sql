/*
  # Sistema de Reagendado de Slots Receptivos con Verificacion de Cupo

  ## Descripcion
  Implementa el flujo completo para reagendar slots receptivos cuando hay conflicto de cupo
  en la fecha/hora destino. Incluye un periodo de respuesta de 12 horas para viajeros afectados.

  ## Nuevas Tablas

  ### 1. `slot_reschedule_requests`
  - Registra una solicitud de reagendado de slot con conflicto de cupo
  - Contiene el tipo de resolucion elegido por la agencia (nuevo horario o ampliar capacidad)
  - Tiene un deadline de 12 horas para que los viajeros respondan

  ### 2. `slot_reschedule_responses`
  - Respuesta individual de cada viajero afectado
  - Estados: pending, accepted, rejected, auto_accepted
  - Al rechazar se procesa reembolso 100% en ToursRed Cash

  ## Modificaciones a Tablas Existentes

  ### `bookings`
  - `has_pending_slot_reschedule` (boolean) - indica si tiene un reagendado de slot pendiente
  - `slot_reschedule_response` (text) - respuesta del viajero al reagendado de slot
  - `slot_reschedule_responded_at` (timestamptz) - cuando respondio

  ## Seguridad
  - RLS habilitado en ambas tablas nuevas
  - Agencias pueden ver/crear/actualizar sus propias solicitudes
  - Viajeros pueden ver solicitudes que afectan sus reservas y actualizar sus respuestas
*/
