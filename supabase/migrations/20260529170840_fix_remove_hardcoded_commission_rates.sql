/*
  # Eliminar porcentajes hardcodeados de las funciones de comisión

  ## Resumen
  Reemplaza todos los fallbacks legacy del MVP (0.10 = 10% y 0.03 = 3%) en las
  funciones financieras con lecturas dinámicas desde las tablas de configuración:
  - `agencies.commission_rate` para la tasa de comisión específica de cada agencia
  - `platform_settings.agency_commission_percentage` como default cuando la agencia
    no tiene tasa configurada
  - `platform_settings.service_charge_percentage` para el cargo por servicio

  ## Funciones modificadas
  1. `calculate_payment_breakdown` — ahora acepta rates opcionales; si no se pasan, lee platform_settings
  2. `calculate_commission_breakdown` — defaults cambiados de 0.10/0.03 a NULL; si son NULL, lee platform_settings
  3. `create_commission_records_for_tour` — guarda el rate real de la agencia y el de platform_settings
  4. `create_commission_records_for_receptivo_slot` — igual que el anterior
  5. `record_booking_financial_transaction` (trigger) — fallback lee agencia + platform_settings
  6. `record_cancellation_financial_transaction` (trigger) — igual

  ## Notas importantes
  - Los commission_records ya existentes NO se modifican (guardan el rate del momento)
  - No hay cambio en la firma pública de calculate_commission_breakdown para llamadas
    que ya pasan rates explícitos (compatibilidad hacia atrás mantenida)
  - La función helper interna `get_effective_rates` lee siempre los valores vigentes
*/
