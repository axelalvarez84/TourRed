/*
  # Service Desk — Sistema de Tickets de Soporte

  ## Resumen
  Crea el módulo completo de soporte técnico (Service Desk) para la plataforma ToursRed.
  Permite a viajeros, agencias y usuarios no registrados levantar tickets de soporte,
  y a administradores/agentes gestionarlos desde un panel centralizado.

  ## Nuevas Tablas

  ### support_categories
  - Categorías de soporte (ej: Pagos, Registro, Tours)
  - Campos: nombre, descripcion, activa

  ### support_subcategories
  - Subcategorías con configuración detallada
  - Campos: category_id, nombre, descripcion, nomenclatura (para folio), prioridad_default,
    sla_horas, aplica_a (array: traveler/agency/general), permite_adjuntos, activa

  ### support_tickets
  - Ticket principal con folio único por nomenclatura
  - Campos: folio, tipo, category_id, subcategory_id, prioridad, status,
    user_id (nullable), solicitante_nombre, solicitante_email, descripcion,
    agente_asignado_id, agencia_asignada_id, ticket_relacionado_id (self-ref),
    created_at, updated_at, closed_at

  ### support_ticket_comments
  - Comentarios internos y respuestas al usuario
  - Campos: ticket_id, author_id (nullable), author_name, tipo (interno/respuesta_usuario), contenido

  ### support_ticket_attachments
  - Archivos adjuntos/evidencias de los tickets
  - Campos: ticket_id, storage_path, nombre_archivo, mime_type, tamano_bytes, subido_por_id

  ### support_ticket_history
  - Línea de tiempo / auditoría de eventos del ticket
  - Campos: ticket_id, tipo_evento, descripcion, actor_id, actor_name, metadata (jsonb)

  ### support_agent_permissions
  - Roles de soporte para usuarios admin
  - Campos: user_id, rol_soporte (super_admin/supervisor/agente/lectura), activo

  ## Funciones SQL
  - generate_ticket_folio(p_subcategory_id): genera folio consecutivo único (ej: PA-0000001)

  ## Seguridad
  - RLS habilitado en todas las tablas
  - Viajeros/agencias ven solo sus propios tickets
  - Agentes ven tickets asignados a ellos o sin asignar
  - Supervisores y super_admin ven todos los tickets
*/
