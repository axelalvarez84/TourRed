/*
  ...
  1. New Tables
    - `discount_codes`
      - `id` (uuid, primary key)
      - `code` (text, unique, the discount code users will enter)
      - `description` (text, description of the discount)
      - `discount_type` (text, type: 'tour_percentage', 'tour_fixed', 'membership_free_month', 'gift_card_percentage', 'gift_card_fixed')
      - `discount_value` (numeric, percentage or fixed amount value)
      - `applicable_to` (text, where it applies: 'tours', 'memberships', 'gift_cards')
      - `is_single_use` (boolean, if the code can only be used once total)
      - `is_active` (boolean, if the code is currently active)
      - `valid_from` (timestamptz, start date of validity)
      - `valid_until` (timestamptz, end date of validity)
      - `max_uses` (integer nullable, maximum total uses, null = unlimited)
      - `times_used` (integer, counter of how many times it has been used)
    ...
    - `discount_code_usage`
      ...
      - `used_at` (timestamptz)
      - `booking_id` (uuid nullable, if used in a booking)
      - `gift_card_id` (uuid nullable, if used in a gift card)
      - `membership_id` (uuid nullable, if used for membership)
      - Unique constraint: user cannot use same code more than once

  2. Security
    - Enable RLS on both tables
    - Admins can create, edit, and delete codes
    - Authenticated users can view active and valid codes
