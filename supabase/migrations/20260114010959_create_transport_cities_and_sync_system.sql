/*
  # Create Transport Cities and Sync System
  
  1. New Tables
    - `cities`
      - Stores information about supported cities
      - Includes bounding box coordinates for OSM queries
      - Tracks enabled/disabled status
    
    - `transport_systems`
      - Stores transport system definitions per city
      - Includes OSM tag queries for each system
      - Supports multiple systems per city
    
    - `osm_sync_logs`
      - Tracks synchronization executions
      - Records statistics and errors
      - Helps monitor system health
  
  2. Initial Data
    - Seeds 8 major Mexican cities with bounding boxes
    - Adds transport systems for each city
    - Metro, Metrobús, Tren Ligero, bus stations, etc.
  
  3. Security
    - Enable RLS on all tables
    - Public read access for cities and transport systems
    - Only admins can modify or trigger syncs
*/
