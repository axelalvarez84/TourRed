/*
  # Sistema de Seguimiento de No Show

  ## Descripción
  Implementa un sistema para trackear cuando los viajeros no se presentan a sus tours.
  Permite a las agencias marcar reservas como "No Show" y mantiene un contador por usuario.
  Los viajeros con más de 3 No Shows se consideran de alto riesgo y deben pagar el 100% del tour.

  ## Cambios

  ### 1. Tabla `bookings`
  - Se agrega el campo `is_no_show` (boolean) para indicar si el viajero no se presentó
  - Se agrega el campo `no_show_marked_at` (timestamptz) para registrar cuándo se marcó como no show
  - Se agrega el campo `no_show_marked_by` (uuid) para saber qué usuario de la agencia lo marcó

  ### 2. Tabla `users`
  - Se agrega el campo `no_show_count` (integer) para contar cuántos no shows ha tenido el viajero
  - Valor por defecto: 0

  ### 3. Función Trigger
  - Se crea una función que actualiza automáticamente el contador de no_show_count cuando se marca una reserva como no show
  - Se crea un trigger en la tabla bookings que ejecuta esta función

  ## Notas Importantes
  - Un viajero con más de 3 no shows debe pagar el 100% del tour en futuras reservas
  - La agencia recibe su pago normalmente incluso cuando hay un no show
  - Solo las reservas confirmadas o completadas pueden marcarse como no show
*/
