/*
  # Sistema de Términos y Condiciones Versionados

  ## Resumen
  Implementa un sistema completo para gestionar, versionar y auditar la aceptación
  explícita de Términos y Condiciones por parte de viajeros y agencias.

  ## Tablas Nuevas

  ### terms_versions
  - Almacena cada versión publicada de los T&C (viajeros o agencias)
  - Solo una versión puede estar activa por tipo en cada momento
  - El contenido es HTML rico editable desde el admin

  ### terms_acceptances
  - Registro de auditoría legal INMUTABLE (sin UPDATE ni DELETE)
  - Captura: fecha/hora, correo, versión, IP, user-agent, user_id
  - Para cumplimiento LFPDPPP y evidencia legal

  ## Columnas en users
  - accepted_traveler_terms_version: última versión aceptada por viajero
  - accepted_agency_terms_version: última versión aceptada por agencia

  ## Funciones SQL
  - get_active_terms(p_type): retorna versión activa de un tipo
  - publish_new_terms_version(...): publica nueva versión y desactiva la anterior

  ## Seguridad (RLS)
  - terms_versions: lectura pública de versiones activas; solo admins gestionan
  - terms_acceptances: INSERT propio; SELECT solo admins; sin UPDATE/DELETE
*/
