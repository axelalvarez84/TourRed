...
-- Replace audit trigger function with hash-based secret redaction.
-- Instead of '[REDACTED]' (same for all values), we now emit
-- '[REDACTED:XXXXXXXX]' where XXXXXXXX is the first 8 hex chars of
-- SHA-256(value). Identical values produce identical hashes (diff is clean),
-- different values produce different hashes (diff shows the change).
-- NULL values produce '[REDACTED:empty]'.
...
BEGIN
  ...
  BEGIN
    -- Resolve actor: prefer auth.uid(), fall back to NEW.updated_by, then 'system'
    v_actor_id := auth.uid();
    ...
    -- Hash-based redaction: produces '[REDACTED:XXXXXXXX]' where XXXXXXXX is
    -- the first 8 hex chars of SHA-256(value). Same value → same hash (diff
    -- is clean). Different value → different hash (diff shows the change).
    FOREACH v_col IN ARRAY k_secret_cols LOOP
      IF v_old_json ? v_col THEN
        ...
        ELSE
          v_hash := '[REDACTED:' || substring(encode(digest(v_raw_val, 'sha256'), 'hex') FROM 1 FOR 8) || ']';
        END IF;
...
-- Re-create trigger (DROP IF EXISTS is idempotent)
...
-- Revoke direct public execution; trigger fires as SECURITY DEFINER
