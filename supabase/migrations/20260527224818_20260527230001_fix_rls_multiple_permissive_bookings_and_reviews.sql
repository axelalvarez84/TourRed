/*
  # Fix Multiple Permissive Policies — Reservas, Reseñas y Slots

  ## Problema
  Múltiples políticas permisivas para el mismo rol y acción se evalúan todas y
  se combinan con OR, generando overhead innecesario en el query planner.

  ## Solución
  Consolidar N políticas del mismo rol/acción en 1 sola política con condiciones OR.
  La lógica de acceso no cambia.

  ## Tablas modificadas
  1.  bookings — SELECT (4→1), UPDATE (2→1)
  2.  booking_cancellations — SELECT (3→1)
  3.  booking_checkin_tokens — SELECT (3→1)
  4.  booking_optional_services — SELECT (3→1)
  5.  booking_partial_cancellations — SELECT (3→1)
  6.  booking_reschedule_responses — SELECT (2→1)
  7.  booking_travelers — SELECT (3→1)
  8.  cancellation_penalty_records — SELECT (3→1)
  9.  agencies — UPDATE (2→1)
  10. agency_reviews — SELECT (2→1), UPDATE (2→1), DELETE (2→1)
  11. traveler_reviews — SELECT (3→1), UPDATE (2→1), DELETE (2→1)
  12. reviews — UPDATE (2→1)
  13. slot_reschedule_requests — SELECT (3→1)
  14. slot_reschedule_responses — SELECT (3→1)
  15. slot_seat_status — INSERT (2→1), UPDATE (2→1)
*/
