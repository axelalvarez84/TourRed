/*
  # Comisión personalizada por tour (override)

  ## Resumen
  Permite configurar una tasa de comisión específica para cada tour,
  con fecha de expiración opcional y motivo documentado.

  ## Cambios en tabla `tours`
  - `commission_rate_override` (numeric, nullable): tasa personalizada 0-50%. NULL = sin override.
  - `commission_override_expires_at` (timestamptz, nullable): cuándo expira el override. NULL = vigente toda la vida del tour.
  - `commission_override_reason` (text, nullable): motivo de la comisión especial, visible para admin y agencia.

  ## Funciones actualizadas
  - `get_effective_commission_rates`: ahora acepta `p_tour_id` opcional y consulta el override del tour primero.
  - `create_commission_records_for_tour`: pasa el tour_id al resolver las tasas.
  - `create_commission_records_for_receptivo_slot`: pasa el tour_id al resolver las tasas.

  ## Jerarquía de resolución
  1. tour.commission_rate_override (si existe y no ha expirado)
  2. agencies.commission_rate
  3. platform_settings.agency_commission_percentage
  4. Fallback hardcoded 0.15

  ## Seguridad
  - No se modifica RLS de tours (ya existente).
  - La función helper usa SECURITY DEFINER para acceder a las tablas sin recursión.
*/
