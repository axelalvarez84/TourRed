...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'bookings' AND column_name = 'service_charge_discount'
  ) THEN
    ALTER TABLE bookings ADD COLUMN service_charge_discount numeric DEFAULT 0 CHECK (service_charge_discount >= 0);
  END IF;
END $$;
