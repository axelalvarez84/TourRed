/*
  # Sistema de Check-in por QR para Reservas

  ## Descripción
  Implementa un sistema de check-in con código QR que permite a las agencias confirmar
  la asistencia de los viajeros el día del tour. El QR se genera al confirmar la reserva
  y expira 24 horas después del inicio del tour.

  ## Cambios

  ### 1. Nueva Tabla `booking_checkin_tokens`
  - Almacena tokens únicos para el QR de check-in de cada reserva
  - `token` (uuid) - token único que se codifica en el QR
  - `booking_id` (uuid) - referencia a la reserva
  - `expires_at` (timestamptz) - 24h después del inicio del tour
  - `redeemed_at` (timestamptz) - cuando la agencia confirmó el check-in
  - `created_at` (timestamptz)

  ### 2. Tabla `bookings` - Nuevas columnas
  - `checkin_status` (text) - null, 'full' o 'partial'
  - `checkin_at` (timestamptz) - timestamp del check-in

  ### 3. Tabla `booking_travelers` - Nueva columna
  - `is_no_show` (boolean) - marca individual de no presentación por acompañante

  ## Seguridad
  - RLS habilitado en `booking_checkin_tokens`
  - El viajero dueño puede leer su token
  - La agencia del tour puede leer y actualizar el token
  - El service role puede insertar tokens
*/
